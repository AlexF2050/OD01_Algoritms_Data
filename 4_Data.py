...
import math                                         # импортируем модуль math для работы с математикой

def is_prime(number):                               # функция для проверки числа на простоту
   if number <= 1:                                  # если число меньше 1, то возвращаем false
       return False
   for i in range(2, int(math.sqrt(number)) + 1):   # создаем цикл, который перебирает все числа от 2 до квадратного корня числа включительно
       if number % i == 0:                          # проверяем делится ли наше число на какое-то из чисел, которые перебирает цикл без остатка
